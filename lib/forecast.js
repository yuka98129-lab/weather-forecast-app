...
/**
 * その日の気温・湿度・降水確率から、洗濯物が乾きやすいかどうかを判定する
 * (新規APIは使わず、既に取得済みの予報データだけで計算する簡易指数)
 */
export function getLaundryAdvice(day) {
  // 降水確率が高い日はそもそも外に干せないので、スコア計算より先に判定する
  if (day.maxPop >= 50) {
    return {
      level: "室内干し推奨",
      advice: "降水確率が高いので、外干しは避けた方が安心です",
    };
  }

  let score = 0;

  if (day.maxTemp >= 25) score += 2;
  else if (day.maxTemp >= 18) score += 1;

  if (day.avgHumidity < 50) score += 2;
  else if (day.avgHumidity < 70) score += 1;

  if (day.maxPop < 20) score += 2;
  else if (day.maxPop < 50) score += 1;

  if (score >= 5) {
    return { level: "よく乾く", advice: "外干しに最適な一日です" };
  }
  if (score >= 3) {
    return { level: "まあまあ乾く", advice: "外干しできますが、乾きは普通程度です" };
  }
  return {
    level: "乾きにくい",
    advice: "室内干しや部屋干し用洗剤の使用がおすすめです",
  };
}
...
export function buildSpeechText(day, cityName) {
  ...
  const laundry = getLaundryAdvice(day);
  const laundryText = `洗濯指数は${laundry.level}です。`;
  ...
  return (
    ...
    `降水確率は${day.maxPop}パーセントです。${uviText}${laundryText}${advice}。`
  );
}
